...
const getLocation = (searchQuery, callback) => {
  ...
  request(url , (error, response, body) => {
    ...
    const cityName = locationInfo.place_name;
    ...
    if (error) {
      ...
      callback(undefined, {cityName, coordinates});
    }
  });
};
...
const getWeatherByString = (cityName = 'Kiev', callback) => {
  getLocation(cityName, (error, location) => {
    if (error) {
      callback(error);
    } else {
      getWeather(location.coordinates, (error, weather) => {
        if (error) {
          callback(error);
        } else {
